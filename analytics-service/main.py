...
def get_category_analytics(transactions: List[TransactionPayload]):
    ...
    return {category: float(np.round(value, 2)) for category, value in category_breakdown.items()}
...
def get_monthly_forecast(transactions: List[TransactionPayload]):
    ...
    if len(monthly_data) < 2:
        ...
        return {
            "predicted_next_month_spending": float(np.round(abs(fallback_prediction), 2)),
